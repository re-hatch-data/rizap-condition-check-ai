...
```
Cloud Scheduler(毎朝 JST 7:00〜8:55、5分おきに起動。SOXAI Ring同期の完了を自動検知)
  ↓
...
以降はCloud SchedulerがJST7:00〜8:55の間5分おきに起動し、SOXAI Ring同期の完了を
自動検知して実行する（「毎朝何時に動かすか」を決める必要はない。詳細は
...
- 起動はJST7:00〜8:55の5分おき。SOXAI Ring同期の完了(データ到着)を自動検知して
  処理するため、固定時刻を決める必要がない(`deploy/deploy.sh` の `SCHEDULE` で変更可能)。
