...
def process_subject(gc, sheets_svc, genai_client, subject: dict) -> None:
    ...
    for _, row in df.iterrows():
        ...
        if cached and cached["comment"] and (cached["hash"] == row_hash or row[DATE_COLUMN] < cutoff):
            comments_by_date[date_str] = cached["comment"]
            continue
        ...
        comments_by_date[date_str] = comment
        generated_count += 1
        logger.info("  [%s] 新規生成: %s", date_str, comment)

    logger.info("  対象%d件中 新規/更新 %d件（残りはキャッシュを再利用）", len(comments_by_date), generated_count)

    if settings.dry_run:
        logger.info("  DRY_RUN: 書き込みはスキップします。")
        return

    write_comment_column(sheets_svc, sh, DAILY_SHEET, COMMENT_COLUMN_HEADER, comments_by_date)
    store.flush()
    logger.info("  反映完了。")
