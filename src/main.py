...
Cloud Run Jobs から Cloud Scheduler 経由で毎朝7:00〜8:55(JST)に5分おきに起動され、
当日分を生成済みの実行は被験者ごとのログシート確認だけで即終了する（冪等）。
"""
...
def process_subject(gc, genai_client, subject: dict, training_start_dates: dict[str, str]) -> None:
    ...
    # 5分おきのポーリング起動のため、当日分を生成済みならこれ以上読まずに抜ける。
    # SOXAI_daily等の再読み込みを省く（APIクォータ節約）と同時に、生成が5分を超えた
    # 朝に後続の実行が同じ日付を二重生成してログシートを書き戻し合う事故を防ぐ。
    # 当日中のデータ訂正はこのスキップにより反映されないが、翌朝の実行で
    # ハッシュ不一致として再生成される
